    ...
    public class RemoveExistHandle : BaseHandle
    {
        ...
        protected override void HandleInternal()
        {
            using (var client = new HttpClient())
            {
                ...
                var response = client.GetAsync($"https://gitee.com/api/v5/repos/{Context.Parameter.Owner}/{Context.Parameter.Repo}/releases?access_token={Context.Parameter.GiteeToken}").GetAwaiter().GetResult();
                ...
                list.ForEach(x =>
                {
                    var deleteResponse = client.DeleteAsync($"https://gitee.com/api/v5/repos/{Context.Parameter.Owner}/{Context.Parameter.Repo}/releases/{x.Id}?access_token={Context.Parameter.GiteeToken}").GetAwaiter().GetResult();
